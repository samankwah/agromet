...
const AgrometAdvisoryUpload = ({ isOpen, onClose, onSave }) => {
  ...
  const generateSheetSummary = (headers, data) => {
      ...
      headers.forEach((header, index) => {
        ...
        if (value) {
          ...
          if (headerLower.includes('activity') || headerLower.includes('stage')) {
            summary.activities.add(value.toString());
          }
          if (headerLower.includes('week') || headerLower.includes('time')) {
            summary.weeks.add(value.toString());
          }
          if (headerLower.includes('crop') || headerLower.includes('commodity')) {
            summary.crops.add(value.toString());
          }
        }
      });
    });
    ...
    return {
      ...summary,
      activities: Array.from(summary.activities),
      weeks: Array.from(summary.weeks),
      crops: Array.from(summary.crops)
    };
  };
  ...
  const generatePreviewData = () => {
    if (!formData.regionCode || !formData.districtCode || !formData.commodityCode) return null;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Add basic information with codes
      ...
      formDataToSubmit.append('commodityCode', formData.commodityCode);
                    ...
                    <div className="p-4">
                      ...
                      <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        <div>
                          ...
                          <span className="ml-1 text-gray-600">{sheet.summary.activities.length}</span>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <span className="ml-1 text-gray-600">{sheet.summary.crops.length}</span>
                        </div>
                        <div>
